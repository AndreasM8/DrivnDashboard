...
function ColumnVisibilityPopover({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-full mr-2 top-0 z-50 shadow-lg rounded-xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 py-2 px-1 min-w-[148px]">
          {hideableCols.map(col => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2 w-full px-2.5 py-1.5 rounded-lg text-left hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
              >
                {/* Checkbox */}
                <span className={`w-3.5 h-3.5 rounded flex-shrink-0 border transition-colors flex items-center justify-center ${
                  visible
                ...
                <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${col.dotColor}`} />
                <span className={`text-xs font-medium ${visible ? 'text-gray-800 dark:text-slate-200' : 'text-gray-400 dark:text-slate-500'}`}>
                  {col.label}
                </span>
              </button>
            )
